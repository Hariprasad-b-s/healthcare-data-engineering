...
| `CLUSTER_ID`                                                          | A fixed base64 UUID identifying the cluster, baked into on-disk storage the first time the broker starts. Hardcoding it makes container recreation deterministic.                                                                                                                               |
...
## 3. Starting and inspecting the broker
...
## 5. Building the producer — and the two problems we hit
...
## 6. The finished producer script
...
## 7. Glossary of the size/throughput knobs we touched
